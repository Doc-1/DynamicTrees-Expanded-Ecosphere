...
public class ButtressRootsGenFeature extends GenFeature {
    ...
    public ButtressRootsGenFeature(ResourceLocation registryName) {
        super(registryName);
    ...
    protected boolean postGrow(@NotNull GenFeatureConfiguration configuration, PostGrowContext context) {
        ...
        if (context.fertility() > 0 && trunkRadius >= configuration.get(MIN_TRUNK_RADIUS)) {
            final CoordUtils.Surround surr = CoordUtils.Surround.values()[level.getRandom().nextInt(8)];
            final BlockPos dPos = treePos.offset(surr.getOffset());
            if (level.getBlockState(dPos).getBlock() instanceof ButtressRootBlock) {
    ...
    protected void nextRoot(LevelAccessor level, SimpleVoxmap rootMap, BlockPos trunkPos, Species species, int trunkRadius, int minTrunkRadius, float scaleFactor, BlockPos pos, int height, int levelCount, Direction fromDir, int radius, int levelLimit) {
        ...
        for (int depth = 0; depth < 2; depth++) {
            ...
            if (pos == BlockPos.ZERO || isReplaceableWithRoots(level, placeState, currPos) && (depth == 1 || onNormalCube)) {
                if (radius > 0) {
                    ButtressRootBlockSupplier.getButtressRoot(species.getFamily()).ifPresent(root ->
                            root.placeRootAtSize(level, currPos, radius, 3)
                    );
                }
                if (onNormalCube) {
    ...
    public ButtressRootsGenFeature setScaler(TetraFunction<Integer, Integer, Integer, Float, Integer> scaler) {
        this.scaler = scaler;
        return this;
    }
}
