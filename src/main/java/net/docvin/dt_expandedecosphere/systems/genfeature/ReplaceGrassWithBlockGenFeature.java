...
public class ReplaceGrassWithBlockGenFeature extends GenFeature {
    ...
    public @NotNull GenFeatureConfiguration createDefaultConfiguration() {
        return super.createDefaultConfiguration().with(BLOCK, Blocks.PODZOL).with(RADIUS, 6).with(GEN_PLACEMENT, 20).with(GEB_PLACE_CHANCE, 0.8F);
    }

    @Override
    protected void registerProperties() {
        this.register(BLOCK, RADIUS, GEN_PLACEMENT, GEB_PLACE_CHANCE);
    }
    ...
    private boolean placeMoss(LevelAccessor level, BlockPos pos, BlockState blockState) {
        for (int y = 0; y <= 6; y++) {
            ...
            if (below.is(Blocks.GRASS_BLOCK) && above.getFluidState().isEmpty()) {
